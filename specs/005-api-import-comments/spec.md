...
- Q: 新影片導入時，留言順序應該是從最舊到最新，還是從最新到最舊？→ A: **策略更新** - 由於 YouTube API 的 `order=time` 參數返回最新優先的留言，且獲取最舊留言需要翻頁到最後（對於留言數多的影片不切實際），因此決定採用**從最新到最舊**的導入策略。首次導入最多 1000 則最新留言，更早的留言可稍後透過「增量更新」功能補充。這樣既符合 API 的自然順序，也避免了不必要的 API 配額消耗。
...
使用者要導入的影片不存在於資料庫，但該頻道已存在。系統從 YouTube API 取得頻道標題、影片標題、影片發布時間、預覽最新 5 則留言（按發佈時間遞減排序），讓使用者確認和修改頻道標籤，然後點擊「確認導入」一次性導入資料。**首次導入策略**：最多導入最新的 1000 則留言（從新到舊），更早的留言可稍後透過「增量更新」功能補充。
...
5. **Given** 使用者確認所有資訊正確，**When** 點擊「確認導入」，**Then** 系統：
   - 呼叫 YouTube API 取得該影片的留言（使用 `order=time`，從最新到最舊，最多 1000 則，遞迴取得所有回覆）
   - 將影片資料存入 videos 表
   - 將所有留言和回覆存入 comments 表
   - 更新 channels 表（不重複插入）
   - 若影片留言數超過 1000 則，顯示提示：「已導入最新的 1000 則留言，更早的留言可透過增量更新補充」
...
使用者要導入的影片和頻道都不存在於資料庫。系統從 YouTube API 取得頻道標題、影片標題、影片發布時間、預覽最新 5 則留言、留言總數，讓使用者選擇新頻道的標籤，然後點擊「確認導入」一次性導入所有資料（頻道、影片、留言）。**首次導入策略**：最多導入最新的 1000 則留言（從新到舊），更早的留言可稍後透過「增量更新」功能補充。
...
- **FR-008a**: **新影片首次導入策略** - 系統必須使用 `order=time` 參數從 YouTube API 獲取留言（返回最新優先），最多導入 1000 則留言（包含回覆），順序為從最新到最舊
- **FR-008b**: 若影片留言總數超過 1000 則，系統必須在導入完成後顯示提示訊息：「已導入最新的 1000 則留言，更早的留言可透過增量更新補充」
