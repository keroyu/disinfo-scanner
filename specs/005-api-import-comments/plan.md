...
**Branch**: `005-api-import-comments` | **Date**: 2025-11-17 | **Last Updated**: 2025-11-18 (Refactoring Update) | **Spec**: `/specs/005-api-import-comments/spec.md`
...
### 2025-11-18 Refactoring Note

本文件對應的 **Y-API (YouTube Official API)** 系統**未進行重新命名**，維持原有清晰的命名慣例。

另一系統 **U-API (urtubeapi)** 已全面重新命名，所有元件加上 `UrtubeApi` 前綴以避免混淆（詳見 `/specs/001-comment-import/`）。現在兩個系統完全獨立：
- **Y-API**: `ImportCommentsController`, `CommentImportService`, `/api/youtube/import/*`, `import-comments-modal.blade.php`
- **U-API**: `UrtubeApiImportController`, `UrtubeApiImportService`, `/api/uapi/*`, `uapi-import-modal.blade.php`

詳細對照表請參考 `/specs/005-api-import-comments/spec.md` 的「2025-11-18 Refactoring Update」章節。
...
2. **U-API (不在此文檔範圍)** = urtubeapi Third-Party *(2025-11-18: 已重新命名)*
   ...
   - **相關檔案** *(已更新)*: `app/Services/UrtubeApiImportService.php`, `app/Services/UrtubeApiMetadataService.php`, `app/Http/Controllers/UrtubeApiImportController.php`
   - **路由前綴**: `/api/uapi/*` (previously `/api/import`)
